...
exports.getBookings = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json(
        generateResponse(200, true, "Bookings retrieved successfully", bookings)
      );
  } catch (error) {
    next(error);
  }
...
exports.postBooking = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(201)
      .json(
        generateResponse(201, true, "Booking created successfully", booking)
      );
  } catch (error) {
    console.log(error);
    next(error);
...
exports.deleteBooking = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json(generateResponse(200, true, "Booking deleted successfully"));
  } catch (error) {
    next(error);
  }
};
...
exports.patchBooking = async (req, res, next) => {
  const { id } = req.params;
  const { pickupDate, pickupTime, dropDate, dropTime, pickupOTP, status } =
    req.body;
